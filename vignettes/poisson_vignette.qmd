---
title: "Frequency analysis of French MTPL dataset"
date: "2024-06-01"
...
: Content of the `CONTRACTS` dataset: contracts {#tbl-dict-freMTPLfreq
.striped .hover}
...
: Content of the `CLAIMS` dataset: claims {#tbl-dict-freMTPLsev .striped
.hover}
:::
...
This leads to inadequate model fit, inflated standard errors, incorrect
...
reference à une autre section\], which offer greater flexibility in
...
ratio of `r exp(coef(reg)[2])` for `DriverAge` (22,26\] implies that the
count of making a claim for individuals aged 22 to 26 are approximately
`r round((1-exp(coef(reg)[2]))*100)`% lower compared to the reference
category. Similarly, count ratios above 1 for `Density` categories
suggest an increase in the count of making a claim as the density
...
::: justify
For more similar databases with a Poisson-like distribution, see
`ausprivauto0405` (import with data("ausprivauto0405")), `brvehins1a`
(import with data("brvehins1a")), or `freMPL1` (import with
data("freMPL1")).
:::
