...
- **Years**: A vector indicating the range of years for which mortality data is available (e.g., 1816-2021).
...
In this vignette, we will focus on the mortality data for males in France. The data is organized to facilitate manipulation and analysis using the `demography` package and other statistical tools available in R, making it well-suited for advanced demographic and actuarial analysis.
:::
...
france <- hmd.mx("FRATNP", "your-email@domain.com", "your-password", "FRA")
```

```{r import, echo=FALSE}

france <- hmd.mx("FRATNP", "julien.siharath@etudiant.univ-rennes1.fr", "19102001Ju@", "FRA")
```
...
plot(france.sm, series = "male", type = "l",
     col = colors, main = "Mortality Rates for Selected Years (Smoothed)",
     ylab = "Mortality Rate", xlab = "Age")


selected_years <- c(1880, 1916, 1943, 2000, 2020)
...
```{r}
#| fig-cap: "Lee-Carter Method: Observed and Forecasted (next 20 years) Male Mortality Rates in France"
...
plot(france, series = "male", ylim = c(-13, 2), lty = 2, lwd = 2, col = "#440154FF",
...
::: justify
The plot displays the smoothed historical male death rates for France from 1816 to 2021, along with forecasted mortality rates. While the Lee-Carter model is a widely utilized tool in actuarial science, the results presented here raise concerns about the accuracy of its forecasts:
...
```{r}
#| code-fold: true
#| fig-cap: "HUrob Method: Mortality rates Observed and Forecasted (next 20 years) Male Mortality Rates in France"
...
legend("bottomright", legend = c("Observed Data", "Forecast"),
       col = c("#440154FF", "#1E88E5"), lty = c(2, 1), lwd = 2, cex = 0.8)
```
The plot displays the log death rates for males in France from 1816 to 2021, across various age groups. The trends indicate that mortality rates have generally decreased over time, particularly at younger ages, but have started to rise again in older age groups.
...
- **Coefficient 1**: ARIMA(1,1,1) with drift. This model suggests moderate positive autocorrelation with a downward trend over time, indicated by the negative drift. The fit is moderate (AIC=639, BIC=652).

- **Coefficient 2**: ARIMA(2,1,3) with drift. A more complex model with mixed coefficient signs indicating intricate relationships. It fits better than Coefficient 1 (AIC=514, BIC=537), with lower residual variance.

- **Coefficient 3**: ARIMA(2,1,2). This model captures variance well (sigma^2=0.339) and has a good fit (AIC=366, BIC=383) without including a drift term.

- **Coefficient 4**: ARIMA(2,1,1). Similar to Coefficient 3 but with a slightly worse fit (AIC=507, BIC=520).

- **Coefficient 5**: ARIMA(0,0,2) with zero mean. A simple model with moderate residual variance (sigma^2=0.419), but its fit is not as strong (AIC=410, BIC=420).

- **Coefficient 6**: ARIMA(1,0,2) with zero mean. This model provides the best fit (AIC=334, BIC=347) with the lowest residual variance (sigma^2=0.289), making it the most suitable among those tested.
