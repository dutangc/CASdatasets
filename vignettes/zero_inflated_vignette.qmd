...
format:
  html:     
    ...
    toc-depth: 2
vignette: >
  %\VignetteIndexEntry{Zero-inflated-Poisson frequency analysis}
...
::: justify
This Zero-inflated regression model predicts NbClaim (number of claims) using `Young`, `Male`, `DistLimit` and `GeoRegion` as predictors.

Count Model (Negative Binomial with Log Link):
The coefficients represent the estimated change in log counts of claims associated with each predictor level compared to a reference level. Most coefficients are statistically significant (p < 0.05), indicating their relevance in predicting claim counts.

Zero-Inflation Model (Binomial with Logit Link):
These coefficients indicate the log-odds of observing excess zeros (zero-inflation) compared to non-excess zeros. Some coefficients, such as Young and Male, are statistically significant (p < 0.05), suggesting their impact on the likelihood of excess zeros.

For example if the insured is `Young`, meaning younger than 26, it increases the log-odds of excess zeros by `r reg$coefficients$zero['Young']`. Whereas `Male` decreases the log-odds by `r abs(reg$coefficients$zero['Male'])`, indicating a lower likelihood of zero-inflation among male drivers.

`DistLimit` have varied impacts on zero-inflation, though most are not statistically significant.

`GeoRegion` showing a non-significant impact.
:::
